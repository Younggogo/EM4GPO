...
if params.lowerisbetter
   state.popranking=ranking(state.popadjustedfitness,state.popsize,0);
   % (use adjusted fitness because ranking is used in the selection process)
else
   state.popranking=ranking(state.popadjustedfitness,state.popsize,1);
end
...
elseif (pop(bestindex).fitness(1) <= state.bestsofar.fitness(1))&&(pop(bestindex).fitness(2) <= state.bestsofar.fitness(2))&&(pop(bestindex).fitness(3) <= state.bestsofar.fitness(3))
    state.bestsofar=pop(bestindex);
end
% || (pop(bestindex).fitness<state.bestsofar.fitness)
...
if params.usetestdata
   ...
   state.bestsofar.Test_RSME=testindividual.Test_RSME;
...
elseif (strcmp(Pruning, 'BestRandom'))    
    if isempty(pop(1).Pruned)|| strcmp(pop(1).Pruned, 'False')         
       ...
       pop(1)= PrunningTheBestRandom(pop(1),data,params,state.generation);
