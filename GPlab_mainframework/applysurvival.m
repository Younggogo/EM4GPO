...
task = 3;
psize = state.popsize;
popfitness=[pop.adjustedfitness]'; % parent population
popf = reshape(popfitness,length(popfitness)/task,task);
tmppopfitness=[tmppop.adjustedfitness]'; % offspring population
tmppopf = reshape(tmppopfitness,length(tmppopfitness)/task,task);
cpopf = [popf;tmppopf];
cpop(1:size(pop,2)) = pop;
cpop((1+size(pop,2)):(size(pop,2)+size(tmppop,2))) = tmppop;

num = 1:1:size(cpop,2);
x1 = [num',cpopf];
f = non_domination_sort_mod(x1,2);
r = f(:,1); % rank
newpop(1:psize)=cpop(r(1:psize));
% -----------------------------------------------
